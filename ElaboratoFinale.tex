...
\setlist[description]{itemindent=0pt}
%%%%% FABIO SMETTE

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% IMPORTANT IMPORTANT IMPORTANT IMPORTANT IMPORTANT IMPORTANT IMPORTANT  IMPOR %
% TANT IMPORTANT IMPORTANT IMPORTANT IMPORTANT IMPORTANT IMPORTANT IMPORTANT I %
% MPORTANT IMPORTANT IMPORTANT IMPORTANT IMPORTANT IMPORTANT IMPORTANT IMPORTA %
% NT IMPORTANT IMPORTANT IMPORTANT IMPORTANT IMPORTANT IMPORTANT IMPORTANT IMP %
% ORTANT IMPORTANT IMPORTANT IMPORTANT IMPORTANT IMPORTANT IMPORTANT IMPORTANT %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% The paragraph before a list (itemize, enumerate or description) must start
% with "\beforelist".
% If the paragraph is longer than one line, use "\beforelist*" instead.
% /!\ The command must be put at the start of the preceding paragraph, NOT   /!\
% /!\ between the paragraph and the start of the list.                       /!\
% A blank line between the paragraph and the start of the list is accepted but
% not necessary.
% 
% Right after the end of a list, put a "\afterlist".
% If the following paragraph is longer than one line, use "\afterlist*" instead.
% /!\ There must be no blank line between the end of the list and the start  /!\
% /!\ of the next paragraph. (Exception: if a chapter/section/ecc... ends    /!\
% /!\ right after the list, put a \afterlist* and a blank line there.        /!\
%
% The \\ command is more often than not EVIL. Don't use it. Never. Ever. Except
% when it's really necessary. But it probably isn't.
% If you think you should use a \\, think again. If you still think like that,
% there is probably a problem in what you're doing: try to find it. If you can't
% find it, YOU're probably the problem.
% There are however a few valid reasons to use it; in doubt, ask.
%
% $blah blah$ is for adding math code "inline". $$blah blah$$ is to typeset
% equations. Both have better versions which are (respectively) \(blah blah\)
% and \[blah blah\].
%
% If you're using \textbf on a word or short sentence before a longer block of
% text that talks about that first word/sentence, you probably aren't trying to
% typeset a normal paragraph.
% If you want to write a list, write a list (\begin{description}
% \end{description}).
% If you want to make small sections, use a sectioning command (\section,
% \subsection, \subsubsection).
% Tell LaTeX what you want it to do, not how to do it. He usually knows better.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% LESS IMPORTANT BUT USEFUL
%
% If you REALLY want to remove the indentation from a paragraph, don't use \\.
% Make it a normal paragraph and start it with \noindent instead.
%
% If you REALLY want to add some vertical space between two lines for no
% particular reason (if there is a reason, use a related command), you COULD use
% \\[space to skip], but \vspace{space to skip} looks nicer.
%
% Newlines in the source code do not do anything (unless you leave a blank
% line), use them! Having shorter lines makes it easier to read the code and to
% spot errors. In ye olde times you couldn't go beyond column 80: it's not a bad
% idea to keep that habit.
%
% If you want to split a piece of text in sections, but you don't want them to
% get numbered, use \section*, \subsection* or \subsubsection*.
%
% If you use a command without arguments (e.g. \textquotedblleft) and you add a
% space after it, that space WILL disappear in the compiled document.
%
% When you are putting an absolute length (e.g. 4cm) in your code, think if
% it could be replaced with a relative length.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\item[Age:] 7\textendash 12 years (or mentally equal in case of cognitive disabilities).
\item[Limitations:]
  \begin{enumerate}
    \item[]
...
\begin{itemize}
\item Battery slot \textemdash ensures that the user's only contact with batteries is through the connectors pinned on the wood;
\item Circuitry slot \textemdash accommodates all circuitry and it is connected above to batteries and below to motors. Also lodges rear LEDs, ON/OFF switch and bumper structure (micro switches with plaques on the outside);
\item Motor slot \textemdash used to store the two motors, which are secured to the wood to ensure stability, and front LEDs;
\item Bottom layer \textemdash serves as lid for the stack and as a base for a small spinning wheel that allows the robot to be balanced.
\end{itemize}
\afterlist*
...
\chapter{External display messages}
\label{app:display}
\import{app_display}
